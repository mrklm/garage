<#  build-windows.ps1 — Garage (Windows) v4.4.20
    ...
    - Il genere dist\Garage.exe puis un ZIP dans releases\
#>

[CmdletBinding()]
param(
  [string]$Version = "4.4.20",
...
# --- 1) Verifs fichiers requis ---
Info "Verification des fichiers requis..."
...
if (!(Test-Path $VenvPy)) {
  ...
  if (!(Test-Path $VenvPy)) { Fail "Echec creation venv (.venv\Scripts\python.exe introuvable)." }
  Ok "Venv cree."
} else {
...
if (-not $KeepBuildDirs) {
  ...
  Warn "KeepBuildDirs active: on conserve build/dist/spec."
}
...
if (!(Test-Path $DistExe)) { Fail "Build termine mais dist\Garage.exe introuvable." }
Ok "EXE genere: $DistExe"
...
Info "Creation ZIP: $ZipName"
Compress-Archive -Path $DistExe -DestinationPath $ZipPath -Force
if (!(Test-Path $ZipPath)) { Fail "ZIP non cree: $ZipPath" }
Ok "ZIP cree: $ZipPath"
...
# --- 6) Option: copie dans AppsDir (mise a jour au meme chemin pour garder les raccourcis) ---
if ($AppsDir.Trim() -ne "") {
  if (!(Test-Path $AppsDir)) {
    Info "Creation AppsDir: $AppsDir"
    New-Item -ItemType Directory -Path $AppsDir | Out-Null
  }
  $DestExe = Join-Path $AppsDir "Garage.exe"
  Copy-Item -Force $DistExe $DestExe
  Ok "EXE copie vers: $DestExe"
}
